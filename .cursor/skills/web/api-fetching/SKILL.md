---
name: api-fetching
description: Scope is apps/web (web frontend). Use when implementing or refactoring web API access in this repo. Web code must route requests through the shared Hono RPC client and an owning service module; raw `fetch` and direct `apiClient` calls are forbidden in UI code. Trigger this skill for any API fetching work under `apps/web`.
---
