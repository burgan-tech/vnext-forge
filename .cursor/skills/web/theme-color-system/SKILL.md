...
- **Base ↔ hover contrast** for any **saturated semantic family** (`destructive`, `success`, `info`, `warning`): the base token (`--color-<variant>`) and the hover token (`--color-<variant>-hover`) must differ by a clearly perceptible luminance step, not just a fractional shade. The base sits **lighter / less saturated** than hover (canonical: Tailwind `*-400` for base, `*-600` for hover — e.g. `--color-destructive: #f87171`, `--color-destructive-hover: #dc2626`), so small icon controls (`size-4` chip remove, row-action trash) read as inviting at rest and clearly intensify on hover. Do **not** set base and hover to two near-identical tones (e.g. `red-500` → `red-600`) and then patch the gap inside one component with `color-mix`, opacity tricks, or raw palette overrides — fix the **token pair** so every consumer (`Button variant="destructive"`, `TagEditor` chip remove, etc.) inherits the same rest/hover rhythm.
...
### Saturated semantic icon controls (destructive `Button`, `TagEditor` remove, row-action trash)

Small saturated controls — the `Button` `destructive` variant, `TagEditor` chip remove (`X`), row-action trash icons inside list items, and any equivalent **icon-bearing semantic action** — share **one** rest / hover language. The same rules apply when the analogous control belongs to another saturated family (`success` confirm, `info` action, `warning` action); only the token family changes.

- **Rest:** background `var(--color-destructive)`, border `var(--color-destructive-border)`, icon color `var(--color-destructive-foreground)` (white on destructive). Icon color comes from the **root** via `currentColor` — do not override the icon to `text-destructive-icon` (the dark rose icon token) on a saturated destructive surface; that token is for muted/soft surfaces (`bg-destructive/10`, `bg-destructive-muted`), not for the saturated `bg-destructive` chrome.
- **Hover:** background `var(--color-destructive-hover)`, border `var(--color-destructive-border-hover)`, **icon color unchanged** (still `--color-destructive-foreground`). The intensification is carried by **background and border only**.
- **Transition contract:** animate **`background-color`** and **`border-color`** only (use the Motion scale: `duration-150` / `duration-200`, `ease-out`). Do **not** include `color` in the `transition` list for these controls — keeping the icon color stable across rest → hover prevents the “icon flashes a different hue” effect and is what makes the trash and chip-remove micro-interactions read as intentional.
- **Two-state visibility (row-action pattern):** in list rows where a destructive action (e.g. row trash) appears only on row hover, scope visibility through the row's `group` (`opacity-0 group-hover:opacity-100`). Visibility belongs to the **row**; the button's own rest / hover colors still follow the rules above.
- **Token-pair dependency:** this whole pattern only feels right when `--color-<variant>` and `--color-<variant>-hover` already differ by a clearly perceptible luminance step (see *Base ↔ hover contrast* in *Token Mapping Guidance*). If a destructive (or other saturated) action looks "almost the same" before and after hover, fix the **token pair** in `apps/web/src/index.css`, not the consumer.
- **Do not** patch one component's perceived rest / hover gap with `color-mix(in srgb, var(--color-<variant>) 88%, white)`, `bg-<variant>/80`, or raw palette utilities (`bg-red-400 hover:bg-red-600`). All consumers of the family must end up with the same rest / hover relationship through the shared token pair.
- **Do not** swap the icon color on hover (e.g. `text-destructive-icon → text-destructive-foreground`) to imitate the same effect — that creates the "color blink" this pattern is designed to avoid.
- **Cancel / dismiss exception still applies:** `cancel`, `close`, and other dismissive controls are not destructive actions and must not adopt this saturated destructive rest / hover language even when they sit next to a destructive control (see *Hover, Border, and Cursor Semantics* main list).
...
- A saturated semantic icon control (`Button variant="destructive"`, `TagEditor` chip remove `X`, row-action trash, or the `success` / `info` / `warning` equivalents) animates **icon color** between rest and hover, or includes `color` in its transition list — the icon must stay on the family's `-foreground` token; only background and border may transition (*Saturated semantic icon controls*).
- A saturated semantic icon control uses the muted/soft icon token (`text-<variant>-icon`) as its rest icon color on top of a saturated `bg-<variant>` surface, instead of `text-<variant>-foreground` (*Saturated semantic icon controls*).
- A consumer patches one control's rest/hover gap with `color-mix`, `bg-<variant>/<n>` opacity tricks, or raw palette utilities (`bg-red-400 hover:bg-red-600`) instead of fixing the **token pair** `--color-<variant>` / `--color-<variant>-hover` so every consumer inherits the same rhythm (*Base ↔ hover contrast*, *Saturated semantic icon controls*).
- The base and hover tokens of a saturated semantic family (`destructive`, `success`, `info`, `warning`) are set to two near-identical tones with no clearly perceptible luminance step between rest and hover (*Base ↔ hover contrast*).
