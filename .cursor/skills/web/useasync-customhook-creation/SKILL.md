...
description: Scope is apps/web (web frontend). Use when shaping async UI flows in the web app. This repo uses `useAsync` as the shared async primitive; pair it with owning service functions that return `Promise<ApiResponse<T>>` for clean, type-safe async UI contracts. Trigger this skill for any `useAsync` or async UI flow work under `apps/web`.
---
