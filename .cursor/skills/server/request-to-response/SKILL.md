...
description: Scope is apps/server (deprecated Hono BFF; reference archive only). Compact guide for designing, reviewing, or debugging the full HTTP request-to-response lifecycle in this backend. Use when deciding middleware order, route flow, controller boundaries, Zod request parsing, standardized success responses, or where a concern should live in the pipeline. Trigger this skill for any HTTP pipeline work under `apps/server`.
---

# Request To Response Process

> **Scope:** `apps/server` (deprecated Hono BFF; kept as reference archive). This skill applies only to code under `apps/server`. New backend work has moved to `apps/extension/src/handlers/*` and uses postMessage transport instead of HTTP — the middleware patterns here do not apply there.
