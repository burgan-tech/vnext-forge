...
description: Scope is apps/server (deprecated Hono BFF; reference archive only). Use when creating, reviewing, or refactoring API responses in this backend. All responses must use `ApiResponse<T>` from `@vnext-forge/app-contracts`. Success responses are `ApiSuccess<T>`, failures are `ApiFailure`. The error-handler middleware owns failure formatting; route handlers own success formatting. Trigger this skill for any API response shape work under `apps/server`.
---

# Response Standardization

> **Scope:** `apps/server` (deprecated Hono BFF; kept as reference archive). This skill applies only to code under `apps/server`. The `ApiResponse<T>` contract itself is still authoritative for the whole repo (web + extension), but the route-handler-specific guidance below targets `apps/server`.
