...
description: Scope is apps/server (deprecated Hono BFF; reference archive only). Use when designing, reviewing, or implementing error contracts in this backend. Domain and service failures must throw `VnextForgeError`; request validation must use Zod; the error-handler middleware owns the single translation boundary from `ZodError` or `VnextForgeError` to `ApiResponse<never>`. Trigger this skill for any error-contract work under `apps/server`.
---

# Errors And Error Handling Mechanism

> **Scope:** `apps/server` (deprecated Hono BFF; kept as reference archive). This skill applies only to code under `apps/server`. New backend logic now lives in `apps/extension/src/handlers/*` — when working there, use the shared error contract from `@vnext-forge/app-contracts` directly without this skill's middleware-specific guidance.
