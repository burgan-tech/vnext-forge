...
export function createOmniSharpInstaller(deps: OmniSharpInstallerDeps) {
  ...
  async function installCsharpLs(): Promise<string | null> {
    ...
    try {
      ...
      if (e?.stderr?.includes('already installed') || e?.stdout?.includes('already installed')) {
        logger.info('csharp-ls already installed')
        return await findCsharpLs()
      }
      logger.warn({ err: e?.message }, 'csharp-ls installation failed')
      return null
    }
  }
  ...
  async function downloadOmniSharp(): Promise<string> {
    ...
    // Node 20+ supports ReadableStream as an async iterable that pipeline can consume.
  ...
  /**
   * Resolves an LSP server for C# files, preferring `csharp-ls` (a regular
   * dotnet global tool) and falling back to a self-contained OmniSharp build.
   *
   * Resolution order:
   *   1. csharp-ls via CSHARP_LS_PATH / system PATH / `~/.dotnet/tools`
  ...
  return { ensureLspServer }
}

export type OmniSharpInstaller = ReturnType<typeof createOmniSharpInstaller>
