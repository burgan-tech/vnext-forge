...
export interface LspWorkspaceDeps {
  logger: LoggerAdapter
}

export function createLspWorkspaceManager(deps: LspWorkspaceDeps) {
  const { logger } = deps

  async function createLspWorkspace(sessionId: string): Promise<LspWorkspace> {
  ...
  async function updateScriptContent(workspace: LspWorkspace, content: string): Promise<void> {
    const wrappedContent = wrapCsxContent(content)
    await fs.writeFile(workspace.scriptPath, wrappedContent, 'utf-8')
  }

  async function destroyLspWorkspace(workspace: LspWorkspace): Promise<void> {
    try {
  ...
  async function runDotnetRestore(workspacePath: string, sessionId: string): Promise<void> {
    ...
    try {
      ...
      logger.warn({ err: message, sessionId }, 'dotnet restore had warnings/errors')
    }
  }

  return { createLspWorkspace, updateScriptContent, destroyLspWorkspace, runDotnetRestore }
}

export type LspWorkspaceManager = ReturnType<typeof createLspWorkspaceManager>

// ── CSX → CS wrapping (pure helpers) ─────────────────────────────────────────
