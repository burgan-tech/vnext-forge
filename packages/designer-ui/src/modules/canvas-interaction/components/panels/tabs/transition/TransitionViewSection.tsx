...
function inferMode(view: ViewBinding | null | undefined, views: ViewBinding[] | undefined): ViewMode {
  if (views && views.length > 0) return 'rule-based';
  return 'single';
}

const EMPTY_REF: ResourceReference = { key: '', domain: '', version: '1.0.0', flow: 'sys-views' };
const EMPTY_BINDING: ViewBinding = { view: { ...EMPTY_REF }, loadData: false };
...
export function TransitionViewSection({
  ...
  const [mode, setMode] = useState<ViewMode>(() => inferMode(view, views));
  ...
  const addBinding = () => {
    onUpdateViews([...views, { ...EMPTY_BINDING, view: { ...EMPTY_REF } }]);
  };

  const removeBinding = (i: number) => {
    const next = views.filter((_, idx) => idx !== i);
    onUpdateViews(next);
  };
  ...
  return (
    <Section title="Views" count={views.length || (view ? 1 : 0)} defaultOpen={hasContent}>
      <p className="text-[10px] text-muted-foreground mb-2 leading-relaxed">
        Assign a view to this transition. Use rule-based mode for conditional view selection.
      </p>
...
/* ─── Single-view sub-component (now uses ViewBinding shape) ─── */
  ...
  return (
    <div>
      ...
      {!hasView ? (
        <button
          type="button"
          onClick={() => onUpdateBinding({ ...EMPTY_BINDING, view: { ...EMPTY_REF } })}
          className="text-secondary-icon hover:text-secondary-foreground inline-flex min-h-0 cursor-pointer items-center gap-1 text-[11px] font-semibold transition-colors">
          <IconPlus />
          Add view manually
        </button>
...
function ResourceReferenceFields({
  ref_,
  updateField,
}: {
...
/* ─── ViewBinding card (rule-based mode) — collapsible ─── */
        ...
        <div className="px-2.5 pb-2.5 space-y-2 border-t border-border pt-2">
          ...
          <div>
            <CsxEditorField
              ...
              contextName={`${stateKey}-${transitionKey}-view-rule-${index}`}
              label="Rule (optional)"
              stateKey={stateKey}
              listField="transitions"
              index={transitionIndex}
              scriptField={`views[${index}].rule`}
            />
          </div>
