 ...
 *
 * Payload is always reachable — free input is never taken away. View is
 * what gets disabled, with a stated reason, when the contract declares no
 * input view. The rendered view carries no `delegate`: it is a plain form
 * here, and the runner (not the view) owns Invoke — see the `onFormChange`
 * tap added to `PseudoUiOrJsonBlock`/`PseudoUiViewSurface` for exactly this.
 */
