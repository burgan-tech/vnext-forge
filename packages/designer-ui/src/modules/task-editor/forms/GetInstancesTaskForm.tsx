  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Page">
          <Input type="number" value={Number(config.page ?? 1)}
            onChange={(e) => {
              const n = Number(e.target.value);
              onChange((d: any) => { d.page = Number.isFinite(n) ? n : undefined; });
            }}
            size="sm"
            inputClassName="text-xs" />
        </Field>
        <Field label="Page Size">
          <Input type="number" value={Number(config.pageSize ?? 10)}
            onChange={(e) => {
              const n = Number(e.target.value);
              onChange((d: any) => { d.pageSize = Number.isFinite(n) ? n : undefined; });
            }}
            size="sm"
            inputClassName="text-xs" />
        </Field>
