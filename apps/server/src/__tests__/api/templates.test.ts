...
describe('API v1 templates routes', () => {
  it('returns 200 + success envelope for templates/validateScriptStatus', async () => {
    const services = {
      ...emptyServices,
      templateService: {
        checkValidateScript: vi.fn(async () => ({ exists: true })),
      },
    } as unknown as ServiceRegistry;

    const app = buildTestApp(services);
    const res = await app.request('/api/v1/templates/validateScriptStatus', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ projectPath: '/tmp/my-project' }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as { success: boolean; data: { exists: boolean } };
  ...
  it('returns 400 when body is invalid JSON', async () => {
    const services = {
      ...emptyServices,
      templateService: { checkValidateScript: vi.fn() },
    } as unknown as ServiceRegistry;
