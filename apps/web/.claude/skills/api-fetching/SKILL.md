...
1. shared API client (`shared/api/client.ts` — `hc<AppType>('/')`)
...
Every API call returns `ApiResponse<T>`. Use the helpers from `@vnext-forge/app-contracts` instead of manual `if (res.success)` checks:
