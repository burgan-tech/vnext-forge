...
// Colors map directly to --color-trigger-* and --color-final-terminated tokens
// Using hex values here since SVG stroke/fill don't support Tailwind classes
function getEdgeColor(triggerType: number, triggerKind: number, isShared: boolean): string {
  if (isShared) return 'var(--color-final-terminated)'; // muted gray for shared
  switch (triggerType) {
    case 0: return 'var(--color-final-terminated)'; // Manual - muted
    case 1:
      if (triggerKind === 10) return 'var(--color-muted-border)'; // Default auto - subtle
      return 'var(--color-trigger-auto)'; // Auto - emerald
    case 2: return 'var(--color-trigger-scheduled)'; // Scheduled - amber
    case 3: return 'var(--color-trigger-event)'; // Event - violet
      ...
      {d.label && (
        <EdgeLabelRenderer>
          <div
            ...
            className={`px-2.5 py-1 rounded-lg text-[10px] font-medium bg-surface/95 backdrop-blur-sm border transition-all duration-150 ${
              selected
