...
import { decodeFromBase64 } from '../editor/Base64Handler';
  ...
  const savePromises = scripts.map(async (script) => {
    ...
    try {
      await fetch('/api/files', {
        ...
        body: JSON.stringify({ path: absolutePath, content: decoded }),
      });
    } catch (err) {
      console.warn(`Failed to save script ${script.location}:`, err);
    }
  });
    ...
    try {
      await Promise.all([
        ...
        fetch('/api/files', {
          ...
          body: JSON.stringify({
            path: diagFilePath,
            content: JSON.stringify(diagramJson, null, 2),
          }),
        }),
        // Save .csx script files to disk
