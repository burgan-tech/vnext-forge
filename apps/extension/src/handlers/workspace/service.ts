...
import { COMPONENT_DIRS, CONFIG_FILE } from './constants'
import type {
  DirectoryEntry,
  SearchResult,
...
import { WorkspaceAnalyzer, type WorkspaceConfigReadStatus } from './workspace-analyzer'
...
export class WorkspaceService {
    ...
    const drives = await Promise.all(
      driveLetters.map(async (letter) => {
        ...
        try {
          ...
          throw this.toFileError(error, 'WorkspaceService.listWindowsDrives', traceId, { drivePath })
        }
      }),
    )

    return drives.filter((d): d is NonNullable<typeof d> => d !== null) as DirectoryEntry[]
  }
  ...
  createDefaultConfig(domain: string, description?: string): VnextWorkspaceConfig {
    ...
    const desc = description?.trim() || `${normalizedDomain} alan tanımı yapılandırması`
