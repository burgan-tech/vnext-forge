...
import type { VnextWorkspaceConfig, VnextWorkspacePaths } from '@handlers/workspace/types'
...
function getInitScriptPath(): string {
  // The template package is copied to dist/vendor/ by the esbuild build step
  // so it is available as a real file on disk inside the extension bundle.
  // __dirname in the bundled dist/extension.js is the dist/ directory.
  return path.join(__dirname, 'vendor', '@burgan-tech', 'vnext-template', 'init.js')
}
...
export class TemplateService {
  ...
  async applyCustomConfig(
    ...
    void traceId // suppress unused-var lint
  }
