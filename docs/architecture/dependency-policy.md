...
| Package / app | Role |
|---------------|------|
| `@vnext-forge-studio/app-contracts` | Pure types, Zod schemas, `ERROR_CODES`, `VnextForgeError`, shared env parsers (`env/common.ts`). **No** imports from other workspace packages. |
| `@vnext-forge-studio/services-core` | RPC method registry, dispatch, services. Imports **`app-contracts` only** — **no** env parsing. |
| `@vnext-forge-studio/lsp-core` | LSP shared code; consumed by **`apps/extension`** and **`apps/server`** where applicable. |
| `@vnext-forge-studio/designer-ui` | React UI, hooks, host ports/adapters. Imports **`app-contracts`**; **no** server secrets. |
...
For Hono `hc` client typing, `apps/web` may import **`import type { AppType } from '@vnext-forge-studio/server'`** from **one** HTTP API shell module (for example `apps/web/src/shared/api/client.ts`, or the current `apps/web/src/transport/HttpTransport.ts` until that consolidation lands). **Runtime** values, middleware, or handlers from `apps/server` must not be bundled into the web app — only TypeScript types.

**Architecture exception rationale:** Workspace resolution requires `@vnext-forge-studio/server` to appear in `apps/web` `dependencies` or `devDependencies`; that satisfies pnpm’s graph even when the app only consumes types. `import type` is stripped at compile time (`tsc -b` emits no runtime import from it), so no server implementation is bundled. The repo’s `apps/web` ESLint `no-restricted-imports` rule applies app-wide except where explicitly narrowed (e.g. `apps/web/src/shared/api/**` for this type-only edge). `apps/web/tsconfig.json` `references` declare composite build ordering so the server types project is built before consumers that depend on it.

## Lint and tooling enforcement

| Rule / script | Enforces | Notes |
|---------------|----------|--------|
| `apps/web` ESLint `no-restricted-imports` | **Ban** `@vnext-forge-studio/services-core` and deep paths into `packages/services-core` (**R-a6**); allow-list `apps/web/src/shared/api/**` (and the single transport file, if used) for `import type` from `@vnext-forge-studio/server` only | See `apps/web/eslint.config.js`. |
| `apps/web` ESLint patterns | Ban imports from `@vnext-forge-studio/designer-ui/dist/**` | Prevents compiled-path leakage. |
...
- Shared parsers: `@vnext-forge-studio/app-contracts/env/common.ts` (`LogLevelSchema`, `NodeEnvSchema`, `coercedBool`, `csvList`, `isLoopbackHost`, …).
