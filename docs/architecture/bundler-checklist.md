...
Use this when adding a public entry such as `@vnext-forge-studio/designer-ui/editor`.
...
7. **Smoke build** — Run `pnpm --filter @vnext-forge-studio/designer-ui --filter @vnext-forge-studio/web --filter @vnext-forge-studio/extension build` (or full `pnpm -r build`) before merging.
